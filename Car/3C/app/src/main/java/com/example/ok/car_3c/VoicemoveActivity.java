...
public class VoicemoveActivity extends AppCompatActivity implements OnClickListener ,CompoundButton.OnCheckedChangeListener{
    ...
    */
    ...
    private static String TAG = VoicemoveActivity.class.getSimpleName();
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_voicebymove);
    ...
    // 应用程序入口处调用，避免手机内存过小，杀死后台进程后通过历史intent进入Activity造成SpeechUtility对象为null
       ...
       // SpeechUtility.createUtility(VoicemoveActivity.this, SpeechConstant.APPID + "=59df8a58,"+SpeechConstant.FORCE_LOGIN+"=true");
        SpeechUtility.createUtility(VoicemoveActivity.this, SpeechConstant.APPID + "=59df8a58,"+SpeechConstant.FORCE_LOGIN+"=true");
        //1.创建SpeechRecognizer对象，第二个参数：本地听写时传InitListener
        mSharedPreferences = getSharedPreferences(getPackageName(), MODE_PRIVATE);
        mCloudGrammar = readFile(this, "grammar_sample.abnf", "utf-8");
        mIat= SpeechRecognizer.createRecognizer(VoicemoveActivity.this, mInitListener);
    ...
    private com.iflytek.cloud.RecognizerListener mRecognizerListener = new com.iflytek.cloud.RecognizerListener() {
        ...
        public void onResult(com.iflytek.cloud.RecognizerResult results, boolean isLast) {
            ...
            if(mIsAsr){
                ...
                if (text.contains("前"))
                {
                    show("up");input="A";connectedThread.write(input.getBytes());
                }
                if (text.contains("后"))
                {
                    show("down");input="B";connectedThread.write(input.getBytes());
                }
                if (text.contains("左"))
                {
                    show("left");input="L";connectedThread.write(input.getBytes());
                }
                if (text.contains("右"))
                {
                    show("right");input="R";connectedThread.write(input.getBytes());
                }
                if (text.contains("停"))
                {
                    show("stop");input="S";connectedThread.write(input.getBytes());
                }
                Log.d(TAG,"ok");
            }else {
                String text ;
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.iat_genral:
                ...
                break;
            case R.id.turnback:
                Intent Intent2 = new Intent(VoicemoveActivity.this,Operation.class);
                String input="S";
